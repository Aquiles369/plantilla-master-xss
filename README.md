...
<p>Para usar correctameten la Opcion 2 require un recon previo para poder elegir los valores correctos en cada categorias de las x1 hasta la x20, "Requiere conocimiento medios sobre xss recomendado mirar mi repo de recursos con mas de 80+ recursos y categorizados"</p><br>
...
x1_motor_navegador:
    ...
    #57:  QQ Browser (Blink/WebKit híbrido)
    ...
    #72:  Firefox Reality (Servo/Gecko híbrido)
...
x2_metodos_http:
    ...
    #94: BOOTSTRAP
	  ...
	  notes: |
	    - Para PoCs “serios” usa `usar: estandar` y listo.
	    - Si querés caos creativo, `usar: ambos`.
	    - Los métodos en `extra` existen en proxies, CDNs, productos o son inventados; no son RFC.
	    - Si querés compatibilidad hacia atrás con tu numeración (#1, #2, ...), podés
...
x3_caracteres_disponibles:
	   ...
	   ========================
	  #1: "<"   → abre etiqueta HTML (permite inyectar nuevas etiquetas)
	  #2: ">"   → cierra etiqueta HTML (finaliza etiquetas abiertas)
	  #3: "/"   → cierre de etiqueta (</tag>, autocierre <br/>)
	  #4: "="   → asignación de atributos (<img src=x>)
	  #5: "\""  → delimitador de atributos (puede romper y salir del atributo)
	  #6: "'"   → delimitador de atributos (igual que comillas dobles)
	  #7: "`"   → usado en atributos inline y JS (permite template literals)
	  #8: "!"   → usado en <!DOCTYPE> o <!-- comentarios --> (control del parser)
	  #9: "?"   → usado en <?php ?>, ? en URLs (querystring)
	  #10: "#"  → marcador de fragmento en URLs (<a href="#frag">)
	   =========================
	   💻 Carácteres de sintaxis JavaScript
	   =========================
	  #11: "{"   → inicio de bloque de código
	  #12: "}"   → fin de bloque de código
	  #13: "("   → llamada a función o agrupación de expresión
	  #14: ")"   → cierre de llamada de función
	  #15: "["   → acceso a arrays/objetos (obj['x'])
	  #16: "]"   → cierre de acceso a arrays/objetos
	  #17: ";"   → separador de sentencias
	  #18: ","   → separador de parámetros o expresiones
	  #19: ":"   → asignación de propiedades en objetos
	  #20: "+"   → concatenación o suma
	  #21: "-"   → resta o negación
	  #22: "*"   → multiplicación
	  #23: "/"   → división o inicio de regex
	  #24: "%"   → operador módulo
	  #25: "."   → acceso a propiedades (obj.prop)
	  #26: "|"   → OR bitwise o logical pipe
	  #27: "&"   → AND bitwise
	  #28: "^"   → XOR bitwise
	  #29: "~"   → NOT bitwise
	  #30: "?"   → operador ternario (condición ? A : B)
	  #31: ":"   → parte del operador ternario
	  #32: "\\"  → escape de caracteres
	  #33: "$"   → válido en nombres de variables/funciones ($evil())
	   =========================
	   🌐 Carácteres especiales de URL/HTTP
	   =========================
	  #34: "%"   → inicio de secuencias %XX (URL encoding, evasión de filtros)
	  #35: "&"   → separador de parámetros (?a=1&b=2)
	  #36: "?"   → inicio de querystring (/path?x=1)
	  #37: "#"   → marcador de fragmento (/index#x)
	  #38: "+"   → representa espacio (x-www-form-urlencoded)
	  #39: ":"   → separador en esquemas (http:)
	  #40: "@"   → usado en credenciales de URL (user:pass@host)
	   =========================
	   🧙 Carácteres escapables en HTML/JS
	   =========================
	  #41: "&#NN;"   → entidades decimales HTML (&#60; = <)
	  #42: "&#xNN;"  → entidades hexadecimales HTML (&#x3c; = <)
	  #43: "\\xNN"   → escape hexadecimal JS (\x3c = <)
	  #44: "\\uNNNN" → escape unicode JS (\u003c = <)
	  #45: "\\NNN"   → escape octal JS (\074 = <) (no siempre soportado)
	  #46: "&lt;"    → entidad HTML (<)
	  #47: "&gt;"    → entidad HTML (>)
	  #48: "&quot;"  → entidad HTML (")
	  #49: "&apos;"  → entidad HTML (')
	  #50: "&amp;"   → entidad HTML (&)
	   =========================
	   🎭 Carácteres invisibles / raros
	   =========================
	  #51: "\u200B" → zero-width space (no visible, rompe validaciones)
	  #52: "\u200C" → zero-width non-joiner
	  #53: "\u200D" → zero-width joiner
	  #54: "\u2060" → word joiner
	  #55: "\uFEFF" → BOM (Byte Order Mark)
	  #56: "\u180E" → Mongolian vowel separator (obsoleto pero soportado)
	  #57: "\u00A0" → espacio no separable (non-breaking space)
	  #58: "\u202E" → BIDI override (invierte el orden visual del texto)
	   =========================
	   💣 Carácteres de ruptura contextual
	   =========================
	  #59: "\">"    → cierra atributo y abre nueva etiqueta (<img src=x" onerror=...>)
	  #60: "</"     → inicia cierre de etiqueta (</script>)
	  #61: "--!>"   → termina comentario HTML (<!-- X --!>)
	  #62: "-->"    → fin de comentario HTML
	  #63: "]]>"    → cierra bloques CDATA
	  #64: "/**/"   → comentario CSS/JS para insertar sin romper sintaxis
	  #65: "//"     → comentario de línea JS (rompe resto del código)
	  #66: "<!--"   → inicia comentario HTML
	   =========================
	   ⚡ Bonus: separadores multi-contexto
	   =========================
	  #67: ","   → separador de expresiones (ej: (1,alert(1)))
	  #68: "|"   → pipe lógico / filtrado en plantillas
	  #69: "~"   → operador bitwise NO (~alert.length)
	  #70: "^"   → XOR, rompe lógicas raras
	  #71: "¬"   → símbolo lógico NO (no estándar pero aceptado en algunos parsers)
          #*: repetir último bypass
          #**:
...
x4_contextos:
    ...
    #202: html_fuera_del_dom (después de </html>)
...
x8_combinaciones_permitidas:
    ...
    #aa   aplicar entropia en el orden semanticos de etga, atributos, eventos <combinaciones válidas> etc
    #*:   última combinación usada
    #**:
    #a:
    #0:
    #00:
...
x10_polimorficos_xss:
	...
	#7: " onclick=alert(1)//<button ‘ onclick=alert(1)//> */ alert(1)//
	#8: “ onclick=alert(1)//<button ‘ onclick=alert(1)//> */ alert(1)//
...
x12_parsers_sanitizadores:
    ...
    #*:
    #**:
    #a:   generar variantes que evadan múltiples parsers
    #0:
    #00:
    #526+:
```
...
x13_waf:
    ...
    #*:
    #**:
    #a: combinar evasiones para múltiples WAF
    #0:
    #00:
    #526+:
```
...
x16_csp_bypass:
    ...
    #2: trusted_types_bypass (definir TrustedType vacía)
...
x17_ofuscaciones_idiomas:
	...
	#2: Latin-1 Supplement (U+0080–U+00FF) — letras/acentos
	#3: Latin Extended-A (U+0100–U+017F)
	#4: Latin Extended-B (U+0180–U+024F)
	#5: Latin Extended Additional (U+1E00–U+1EFF)
	#6: Greek (U+0370–U+03FF) — subset homoglifos (Α,Β,Ε,Ζ,Η,Ι,Κ,Μ,Ν,Ο,Ρ,Τ,Χ)
	#7: Greek Extended (U+1F00–U+1FFF) — usar poco, compat amplia
	#8: Cyrillic (U+0400–U+04FF) — subset homoglifos (А,В,Е,К,М,Н,О,Р,С,Т,Х,І,Ј)
	#9: Cyrillic Supplement (U+0500–U+052F) — homoglifos extra (limitado)
	#10: Fullwidth Forms LATIN (U+FF01–U+FF5E) — válido en strings/identificadores; evitar operadores
	#11: Mathematical Alphanumeric Symbols — Latin (U+1D400–U+1D7FF) en strings/identificadores
	#12: Mathematical Alphanumeric Symbols — Greek (U+1D6A8–U+1D7FF) en strings/identificadores
	#13: Phonetic Extensions (U+1D00–U+1D7F) — solo strings/ids, buen relleno
	#14: Phonetic Extensions Supplement (U+1D80–U+1DBF) — solo strings/ids
	#15: Combining Diacritical Marks (U+0300–U+036F) — solo dentro de strings (Zalgo light)
	#16: Latin Extended-C/D/E (U+2C60–U+2C7F / U+A720–U+A7FF / U+AB30–U+AB6F) — cobertura extra latina
	#17: Modifier Letters (U+02B0–U+02FF) — strings/ids, ojo legibilidad
	#18: Greek & Coptic confusables adicionales (U+03D0–U+03FF subset) — strings/ids
	#19: Cyrillic Extended-A/B (U+2DE0–U+2DFF / U+A640–U+A69F) — homoglifos raros strings/ids
	#20: Letterlike Symbols (U+2100–U+214F) — strings/ids, algunos útiles tipo ℮, ℓ

 	(solo en strings/identificadores, no en sintaxis/operadores):

	#21: Superscripts & Subscripts (U+2070–U+209F) — strings/ids
	#22: Combining Diacritical Marks Extended (U+1AB0–U+1AFF) — strings/ids (Zalgo light)
	#23: Enclosed Alphanumerics (U+2460–U+24FF) — strings/ids (ojo legibilidad)
	#24: Latin Extended Additional (U+1E00–U+1EFF) — ya lo incluí como #5; si lo separás, mantené una sola vez
	#25: Spacing Modifier Letters (U+02B0–U+02FF) — strings/ids
	#26: Greek & Coptic extra confusables (U+03D0–U+03FF subset) — strings/ids
	#27: Cyrillic Extended-A/B (U+2DE0–U+2DFF / U+A640–U+A69F) — strings/ids
	#28: Mathematical Operators letters-as-symbols útiles (U+2200–U+22FF subset) — strings/ids
...
```yaml
⚠️ (Parciales, no confiables): #20, #21–#22, #23, #24–#26, #33, #39
💀 (No ejecutan): todos los demás (#27–#129, incluidos Cuneiforme, Lineal A/B, Runic, Ogam, Elbasan, etc.)
    #*: usar la última ofuscación empleada
    #**: aplicar la ofuscación elegida a cada carácter del payload uno por uno (excepto los caracteres especiales y mantener la estructura de js etc )
    #a: combinar entre sí todos los parámetros elegidos y aplicar entropia en cada paylodad(#20,#50,#70)
    #0: no aplicar nada de esta categoría
    #00: IA libre (si no hay parámetros definidos)
    #526+: forzar generación de mínimo 526 combinaciones
```
 <br>
...
x18_codificaciones:
    ...
    #*: usar la última codificación usada
    #**: aplicar codificación solo a caracteres normales
...
```yaml
#aa: activar combinaciones cruzadas entre TODAS las categorías activas (modo fusión total)
```
<br>
...
modo_validación: "estricto"   # valores: estricto | flexible | off

# 🧩 1 — Limitaciones para controlar el caos
max_payload_base_length: 80
max_layers_per_payload: 6
min_unique_combinations: 526

# 🧩 2 — Validación de contenido estructural
...
# 🧩 3 — Codificación real por carácter
codificacion:
  aplicar_por_caracter: true
  mezclar_formatos: true
  aplicar_fullwidth_solo_letras: true
  omitir_si_codificado_por_x20: true         # ⚡ evita que #** actúe sobre caracteres ya tocados por x20

# 🧩 4 — Anti-colisión y diversidad
anti_colision:
...
# 🧩 5 — Cobertura total obligatoria
cobertura_total: obligatoria

# 🧩 6 — Codificación híbrida de especiales (x20)
...
Si payload_input ≠ "2":
→ Modo automático: aplicar TODAS las categorías activas (x1–x20) según las combinaciones permitidas
usando #*, #**, #a, #aa, #526+ y #00,
y generar payload_count variantes creativas.

Si payload_input = "2":
→ Modo manual: usar exclusivamente los valores que indique el usuario para cada categoría
(sin añadir, quitar o alterar nada que no haya sido escrito por el usuario).

---

📌 Reglas técnicas obligatorias

- Cada categoría acepta múltiples parámetros separados por `,` (ej: #1,#5,#70)
- #a → combinar todos los parámetros de esa categoría
- #aa → combinar entre TODAS las categorías activas
- #* → reutilizar el último valor elegido automáticamente
- #** → aplicar ofuscación carácter por carácter (excepto especiales y omitiendo los ya tocados por x20)
- #0 → omitir la categoría
- #00 → IA libre si no hay parámetros
- #526+ → generar al menos 526 combinaciones reales
- Se permiten alias x1–x20

---

📌 Reglas de obediencia absoluta

- No puede añadir, quitar, sustituir ni ignorar parámetros
- Cada payload aplica TODAS las categorías activas
- Debe incluir ~10% de payloads en texto plano (sin x20)
- Debe insertar esos payloads de texto plano dispersos entre los demás
- No puede decidir qué codificar: solo lo que indiquen los comandos activos 
---

📌 Reglas de control absoluto

- No usar aleatoriedad libre ni conocimiento implícito
- Si una categoría tiene #526+ → generar al menos esa cantidad de combinaciones reales
- Si tiene múltiples parámetros (#1,#2,#3,#a) → generar combinaciones cruzadas de todos
- No alterar orden de caracteres salvo si la ofuscación lo indica
- Validar cada payload: si falla una regla → descartarlo y regenerar
- Salida: solo los payloads en crudo, uno por línea

---

📌 Reglas de consistencia estructural

- Cada payload debe ser HTML/JS válido completo
- Si hay Mutation XSS → validar que ejecuta JS real tras la mutación
...
- Si es igual o parecido → descartarlo y regenerar

---

📌 Reglas de cobertura total

- Si hay parámetros múltiples (#1,#2,#3,#a) → marcar cuáles ya se usaron
...
✔ Patrón completamente único  
✔ Adaptado al contexto indicado  
✔ ~10% de payloads en texto plano  
✔ Evasión efectiva del parser  
✔ Compatibles con el framework y el WAF indicados  
✔ Alta entropía y creatividad (modo varios Aquiles trabajando a la vez)
